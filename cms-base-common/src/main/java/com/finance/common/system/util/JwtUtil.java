package com.finance.common.system.util;
...
import com.finance.common.constant.DataBaseConstant;
import com.finance.common.exception.JeecgBootException;
import com.finance.common.system.vo.SysUserCacheInfo;
...
public class JwtUtil {
	 ...
	 * @return 是否正确
	 */
	public static boolean verify(String token,String corpCode,String username, String secret) {
		try {
			// 根据密码生成JWT效验器
			Algorithm algorithm = Algorithm.HMAC256(secret);
			String[] userCorp = {corpCode,username};
			JWTVerifier verifier = JWT.require(algorithm).withArrayClaim("userNameCorpCode",userCorp).build();
			// 效验TOKEN
	 ...
	 *
	 * @return token中包含的用户名,0位为corpCode,1位为userName
	 */
	public static String[] getUsername(String token) {
		try {
			DecodedJWT jwt = JWT.decode(token);
			return jwt.getClaim("userNameCorpCode").asArray(String.class);
	 ...
	 * @return 加密的token
	 */
	public static String sign(String corpCode,String username, String secret) {
		Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME);
		Algorithm algorithm = Algorithm.HMAC256(secret);
		// 附带username信息
		String[] signValueKey = {corpCode,username};
		return JWT.create().withArrayClaim("userNameCorpCode",signValueKey).withExpiresAt(date).sign(algorithm);
