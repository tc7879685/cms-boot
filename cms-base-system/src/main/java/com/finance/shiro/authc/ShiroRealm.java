...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		// 校验token有效性
		UserInfo loginUser = this.checkUserTokenIsEffect(token);
	 ...
	 * @param token
	 */
	public UserInfo checkUserTokenIsEffect(String token) throws AuthenticationException {
		// 解密获得username，用于和数据库进行对比
		String[] corpUser  = JwtUtil.getUsername(token);
		if (corpUser == null || corpUser.length == 0) {
			throw new AuthenticationException("token非法无效!");
		}
		//获取用户信息
		String corpCode  = corpUser[0];
		String userName  = corpUser[1];

		// 查询用户信息
		UserInfo loginUser = userInfoService.getUserInfoByName(corpCode,userName);
		if (loginUser == null) {
			throw new AuthenticationException("用户不存在!");
		}

		// 校验token是否超时失效 & 或者账号密码是否错误
		if (!jwtTokenRefresh(token,corpCode ,userName, loginUser.getUserPassword())) {
		...
		//BeanUtils.copyProperties(sysUser, loginUser); //复制
		return loginUser;
	}
	 ...
	 * @return
	 */
	public boolean jwtTokenRefresh(String token,String corpCode, String userName, String passWord) {
		String cacheToken = String.valueOf(redisUtil.get(CommonConstant.PREFIX_USER_TOKEN + token));
		if (oConvertUtils.isNotEmpty(cacheToken)) {
			// 校验token有效性
			if (!JwtUtil.verify(token,corpCode, userName, passWord)) {
				String newAuthorization = JwtUtil.sign(corpCode,userName, passWord);
